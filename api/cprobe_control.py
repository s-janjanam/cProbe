...
class NProbeConstants:
    ...
    CONFIG_PATH = Path("/run/cpacket/config/probe.json")
    PID_FILE_FORMAT = "/opt/nprobe/logs/nprobe-{}.pid"
...
class NProbeController:
    ...
    def _get_default_settings(self):
        return {
            ...
            "capture": {
                "interfaces": [{"name": "enp104s0f0np0", "rss_queues": "auto"}],
                "cluster_id": 99,
                "sample_rate": "1:1:1"
            },
            "processing": {"aggregation": "1.1/1/1/1/0/0/0", "custom_options": {}},
            "export": {
                "targets": ["10.51.10.238:2055"],
                "flow_version": 9,
    ...
    def _build_args_from_config(self) -> list:
        ...
        interfaces = capture_cfg.get('interfaces', [])
        cluster_id = capture_cfg.get('cluster_id')

        # --- MODIFIED LOGIC FOR CORRECT CLUSTER SYNTAX ---
        if interfaces:
            # Get the physical interface name
            iface_name = interfaces[0].get('name')
            if iface_name:
                args.append(f"--interface={iface_name}")
                self.logger.info(f"Setting capture interface to: {iface_name}")

                # If a cluster ID is specified, add it as a SEPARATE argument
                if cluster_id is not None:
                    args.append(f"--cluster-id={cluster_id}")
                    self.logger.info(f"Enabling PF_RING cluster with ID: {cluster_id}")
        # --- END OF MODIFICATION ---
        
        args.append("--cpu-affinity=2-7")
        self.logger.info("Binding nprobe packet threads to hardcoded CPU cores: 2-7")
        
        args.append("--export-thread-affinity=1")
        ...
        args.append(f"--flow-version={export_cfg['flow_version']}")
        args.append(f"-t {export_cfg['idle_timeout_secs']}")
        args.append(f"-d {export_cfg['active_timeout_secs']}")
        args.append(f"-T \"{export_cfg['template']}\"")
    ...
    def start(self) -> bool:
        if self.get_process_status() == "running":
            self.logger.warning("Start command ignored: process is already running.")
        ...
        try:
            ...
            self.logger.info(f"Running nprobe instance, cmd: {' '.join(cmd)}")
