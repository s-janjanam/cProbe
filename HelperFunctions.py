...
def init_dpdk_nics(capture_nic_index):
    """
    Assigning a network interface to DPDK for capture
    :param vm_type:
    :param capture_nic_index:
    :return:
    """
    rc = None
    print("Binding DPDK NIC: {} (base image: {})".format(capture_nic_index, baseimage()))

    num_nics = 0
    if baseimage() == 1804:
        dpdk_drivers = dpdk_devbind.check_modules()
        for driver in dpdk_drivers:
            print("loaded dpdk driver: {}".format(driver))
        devices = dpdk_devbind.get_device_details(dpdk_devbind.network_devices)
        index = 0
        num_nics = len(devices)
        for slot in devices:
            device = devices[slot]
            print("network device: slot={}, interface={}, ssh={}, module={}, driver={}".format(
                device['Slot'],
                device['Interface'],
                device['Ssh_if'],
                device['Module_str'],
                device.get('Driver_str','')))
            if index == capture_nic_index:
                if dpdk_drivers:
                    print("Attaching to dpdk device: {}".format(device['Slot']))
                    dpdk_devbind.bind_one(device['Slot'], dpdk_drivers[0], True)
                    rc = device['Slot']
                break
            index += 1
        else:
            print("DPDK index NIC is outside the valid range: 0 .. {}".format(len(devices)))
    return rc, num_nics
